...
import os
token ='7973772697:AAHcc8r0zIIkIHF_SQKkqQXl-mTupyak4k8'
bot=telebot.TeleBot(token,parse_mode="HTML")
subscriber = '6861279097'
@bot.message_handler(commands=["start"])
def start(message):
	if not str(message.chat.id) == '6861279097':
...
def main(message):
	if not str(message.chat.id) == '6861279097':
		bot.reply_to(message, "Only for KoKo Thu Ra🙄💗")
		return
	dd = 0
	live = 0
	ch = 0
...
◆𝑷𝑹𝑶𝑿𝒀𝑺: 𝑷𝑹𝑶𝑿𝒀 𝑳𝑰𝑽𝑬 ✅ '''
				print(last)
				if 'Thank you for your message.' in last or 'success' in last or 'redirecturl' in last or 'redirect_to' in last or 'thank you' in last or 'payment' in last or 'successfully' in last:
					live += 1
